...
///
/// This is the root directory for all helix-tools data.
///
/// # Environment Override
/// Set `HELIX_HOME` to override the default location.
#[must_use]
...
/// Shared configuration used by multiple helix tools.
///
/// Loaded from `~/.helix/config/config.toml` and `.helix/config.toml`.
...
pub struct StorageConfig {
    /// Base directory for data storage. Defaults to `~/.helix/data`.
    #[serde(default = "helix_data_dir")]
    pub base: PathBuf,
}
...
/// Get the global config directory (`~/.helix/config`).
///
/// This is a convenience alias for [`helix_config_dir`].
#[must_use]
#[deprecated(since = "0.2.0", note = "use helix_config_dir() instead")]
pub fn global_config_dir() -> Option<PathBuf> {
    Some(helix_config_dir())
}

/// Find the project config directory (`.helix/`) by checking cwd.
///
/// Returns `None` if no `.helix/` directory exists in the current working directory.
#[must_use]
