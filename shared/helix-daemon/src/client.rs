...
impl Client {
    ...
    async fn start_daemon(&self) -> Result<(), DaemonError> {
        ...
        let helixd_path = std::env::current_exe()
            .ok()
            .and_then(|p| p.parent().map(|d| d.join("helixd")))
            .filter(|p| p.exists())
            .unwrap_or_else(|| "helixd".into());

        tokio::process::Command::new(helixd_path)
            .arg("--socket")
            .arg(&socket_path)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .map_err(|e| DaemonError::Internal(format!("Failed to start helixd: {e}")))?;
