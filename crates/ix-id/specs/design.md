...
This document describes the design decisions and implementation details for `ix-id`.

## Overview

`ix-id` provides hash-based ID generation for the Ixchel ecosystem. IDs are designed for git-backed, offline-first systems where coordination between machines is not possible.
