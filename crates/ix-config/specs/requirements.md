...
This document defines requirements for `ix-config` using [EARS notation](https://www.iaria.org/conferences2015/filesICCGI15/EARS.pdf).
...
### US-001: Ixchel Home Directory

**As a** developer\
**I want to** access a unified ixchel home directory\
**So that** global ixchel config/state/data is organized in one place

| ID       | Acceptance Criterion                                                           |
| -------- | ------------------------------------------------------------------------------ |
| AC-001.1 | THE SYSTEM SHALL use `~/.ixchel/` as the default ixchel home directory         |
| AC-001.2 | THE SYSTEM SHALL respect `IXCHEL_HOME` environment variable if set             |
| AC-001.3 | THE SYSTEM SHALL provide `ixchel_home()` function returning the home path      |
| AC-001.4 | THE SYSTEM SHALL create the home directory if it does not exist (when writing) |

---

### US-002: Ixchel Subdirectories

**As a** developer\
**I want to** access specific ixchel subdirectories\
**So that** I can organize config, data, state, and logs

| ID       | Acceptance Criterion                                                         |
| -------- | ---------------------------------------------------------------------------- |
| AC-002.1 | THE SYSTEM SHALL provide `ixchel_config_dir()` returning `~/.ixchel/config/` |
| AC-002.2 | THE SYSTEM SHALL provide `ixchel_data_dir()` returning `~/.ixchel/data/`     |
| AC-002.3 | THE SYSTEM SHALL provide `ixchel_state_dir()` returning `~/.ixchel/state/`   |
| AC-002.4 | THE SYSTEM SHALL provide `ixchel_log_dir()` returning `~/.ixchel/log/`       |
...
**As a** developer\
**I want to** load configuration for my Ixchel tool\
...
| ID       | Acceptance Criterion                                                                  |
| -------- | ------------------------------------------------------------------------------------- |
| AC-004.1 | THE SYSTEM SHALL read global shared config from `~/.ixchel/config/config.toml`        |
| AC-004.2 | THE SYSTEM SHALL read tool-specific global config from `~/.ixchel/config/<tool>.toml` |
...
| ID       | Acceptance Criterion                                                            |
| -------- | ------------------------------------------------------------------------------- |
| AC-005.1 | THE SYSTEM SHALL read project shared config from `.ixchel/config.toml`          |
| AC-005.2 | THE SYSTEM SHALL read tool-specific project config from `.ixchel/<tool>.toml`   |
| AC-005.3 | THE SYSTEM SHALL search for `.ixchel/` by walking up to the git repository root |
| AC-005.4 | IF no `.ixchel/` directory exists THEN THE SYSTEM SHALL skip project config     |
...
| ID       | Acceptance Criterion                                                                         |
| -------- | -------------------------------------------------------------------------------------------- |
| AC-007.1 | WHEN `IXCHEL_HOME` is set THE SYSTEM SHALL use that as the ixchel home directory             |
| AC-007.2 | WHERE `HELIX_HOME` is set THE SYSTEM SHALL use that as a legacy fallback                     |
| AC-007.3 | THE SYSTEM SHALL convert `__` to nested keys (e.g., `IXCHEL_GITHUB__TOKEN` → `github.token`) |
...
**As a** developer\
**I want to** `.ixchel/` to be found relative to working directory\
**So that** project config works from subdirectories

| ID       | Acceptance Criterion                                                    |
| -------- | ----------------------------------------------------------------------- |
| AC-010.1 | THE SYSTEM SHALL search for `.ixchel/` in the current working directory |
