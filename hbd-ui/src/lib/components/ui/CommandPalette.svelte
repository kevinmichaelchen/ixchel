...
{#if open}
	...
	>
		...
		<div class="flex items-center border-b border-border-card px-4">
			...
			/>
			<kbd class="ml-2 hidden rounded border border-border-card bg-muted px-1.5 py-0.5 text-xs text-foreground-alt sm:inline-block">
				ESC
			</kbd>
		</div>
		...
		<div class="max-h-[400px] overflow-y-auto p-2">
			{#if filteredItems.length === 0}
				<div class="py-6 text-center text-sm text-foreground-alt">
					No results found.
				</div>
			{:else}
				<!-- Actions group -->
				{#if actionItems.some((a) => filteredItems.includes(a))}
					...
					{#each actionItems.filter((a) => filteredItems.includes(a)) as item, i}
				...
				{#if statusItems.some((s) => filteredItems.includes(s))}
					...
					{#each statusItems.filter((s) => filteredItems.includes(s)) as item}
						{@const globalIndex = filteredItems.indexOf(item)}
				...
				{#if issueItems.some((issue) => filteredItems.includes(issue))}
					...
					{#each issueItems.filter((issue) => filteredItems.includes(issue)) as item}
						{@const globalIndex = filteredItems.indexOf(item)}
