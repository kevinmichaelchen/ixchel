		...
		switch (e.key) {
			...
			case 'ArrowDown':
				...
				if (filteredIssues.length > 0) {
					...
					const currentIndex = selectedId ? filteredIssues.findIndex((i) => i.id === selectedId) : -1;
			...
			case 'ArrowUp':
				...
				if (filteredIssues.length > 0) {
					const selectedId = selectedIssue?.id;
					const currentIndex = selectedId ? filteredIssues.findIndex((i) => i.id === selectedId) : 0;
	...
	<Tabs bind:value={view} class="flex h-full w-full flex-col">
		<header class="flex h-14 items-center justify-between border-b bg-card px-4">
			...
			<div class="flex items-center gap-2">
				<Button variant="outline" size="sm" onclick={triggerCommandPalette} class="text-muted-foreground">
		...
		<div class="flex-1 overflow-hidden relative">
			{#if error}
				<div class="absolute left-1/2 top-4 z-50 -translate-x-1/2 rounded bg-destructive px-4 py-2 text-sm text-destructive-foreground shadow-lg">
					{error}
				</div>
			{/if}
